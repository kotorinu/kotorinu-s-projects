...
export function workCoverage(tasks: Task[], goals: Goal[], runs: WorkRun[], today: string) {
  const pending = tasks.filter(t => !["ARCHIVED", "DELETED", "SUPERSEDED", "MERGED"].includes(t.lifecycle) && !["Archive", "完了"].includes(t.status));
  const ids = new Set(goals.map(g => g.id));
  const openGoals = goals.filter(g => ["進行中", "未達成"].includes(g.status));
  return {
    pending, overdue: pending.filter(t => t.deadline && t.deadline < today),
