...
export default function TaskDetailSheet(props: ComponentProps<typeof AdvancedView>) {
  ...
  return <StudioDialog title={task.title} onClose={props.onClose}><div className="space-y-6">
    {!work.connected && <p className="rounded-xl bg-amber-50 p-3 text-xs leading-6 text-amber-800">接続前の参考表示です。最新の状況ではありません。</p>}
    ...
    <div className="grid grid-cols-2 gap-4 rounded-2xl bg-[#f8f7fb] p-4 text-sm"><div><p className="mb-2 text-xs text-slate-500">作業する日時</p><p className="leading-7">{next?next.date.replaceAll("-","/")+" "+next.startTime+"〜"+next.endTime:"未設定"}</p>{next && <p className="mt-1 text-xs leading-6 text-slate-500">アプリ内の計画です。Calendarで照合できます。</p>}</div><div><p className="mb-2 text-xs text-slate-500">見積り / 記録した時間</p><p className="leading-7">{task.estimateMinutes===null?"未設定":task.estimateMinutes+"分"} / {actual===null?"未入力":actual+"分"}</p></div></div>
