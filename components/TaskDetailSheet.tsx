...
export default function TaskDetailSheet(props: ComponentProps<typeof AdvancedView>) {
  ...
  // Fixed reading order: state, what it is, when it is done, when it is worked
  // on and what it took, then the action. The goal it belongs to comes after
  // the action — it is why the task exists, not the next thing to press. The
  // actual-time editor sits with the times it edits, rather than below the
  ...
  return <StudioDialog title={task.title} onClose={props.onClose}><div className="space-y-6">
    ...
    <section className="rounded-2xl bg-accent-soft p-5"><h3 className="mb-3 text-sm font-semibold">ここまでできたら完了</h3>{task.definitionOfDone.length?<ul className="space-y-3">{task.definitionOfDone.map((d,i)=><li key={i} className="flex gap-3 text-sm leading-7"><span className="text-stone-400" aria-hidden="true">○</span><span>{d}</span></li>)}</ul>:<p className="text-sm leading-7 text-slate-500">完了条件はまだ設定していません。</p>}</section>
