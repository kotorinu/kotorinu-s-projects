...
import { capabilityOwnerLabel, deliveryStatusLabel } from "@/lib/capability";
...
export default function TaskDetailSheet({
  ...
  const { goals, tasks: allTasks, connected } = useWork();
  ...
  function applyReschedule(args: {
    ...
    if (!connected || !execution.executionReady) return;
  ...
  const goalProgress = goal && connected && execution.executionReady ? computeGoalProgress(allTasks, goal.id, execution) : null;
  ...
  return (
    <StudioDialog title="手順・履歴・詳細設定" onClose={onClose}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-[max(2rem,env(safe-area-inset-bottom))]">
          ...
          {task.linkedSalesMaster && (
            <Section title="営業Master">
              <Link href="/sales-master" className="text-sm font-bold text-accent-dark">
                ＞ 営業Masterを見る
              </Link>
            </Section>
          )}
            ...
            <Section title="上位成果">
              {goal ? (
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm leading-relaxed text-stone-600">{outcome.desiredState}</p>
                  <button
                    type="button"
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <details className="studio-disclosure mt-5 first:mt-4">
