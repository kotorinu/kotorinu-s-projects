...
export default function RunbookStrip({
  ...
  return (
    <div className="mt-1.5 rounded-xl bg-white/70 px-2.5 py-2">
      {current ? (
        <>
          <p className="text-[13px] font-black tracking-widest text-accent-dark">
            いまここ {current.startTime}〜{current.endTime}
          </p>
          <p className="mt-0.5 text-[14px] font-bold leading-snug text-stone-800">{current.label}</p>
          <p className="mt-0.5 text-[13px] leading-relaxed text-stone-500">→ {current.outputs}</p>
        </>
      ) : (
        <p className="text-[13px] font-black tracking-widest text-stone-400">この枠の進め方</p>
      )}

      {next && (
        <p className="mt-1.5 border-t border-stone-100 pt-1.5 text-[13px] leading-snug text-stone-500">
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="mt-1 text-[13px] font-bold text-stone-400"
      >
      ...
      {runbook.note && <p className="mt-1 text-[13px] leading-relaxed text-stone-400">{runbook.note}</p>}
    </div>
  );
}
