    ...
    <div className="flex flex-col pb-8">
      ...
      <section className="mx-5 mt-1 rounded-3xl bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_-12px_rgba(0,0,0,0.12)]">
        ...
        {salesSprint.checkpointDate && (
          <p className="text-[13px] font-bold text-stone-400">{formatMd(salesSprint.checkpointDate)}まで</p>
        )}
        <p className="mt-2 text-[14px] leading-relaxed text-stone-600">{salesSprint.goal}</p>
        ...
        <div className="mt-3 border-t border-stone-100 pt-3">
          <p className="mb-2 text-[13px] font-black tracking-widest text-stone-400">
            9/9の到達目標：UNDERSTANDING
          </p>
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-2xl bg-stone-50 px-3 py-2.5">
              <p className="text-[13px] font-bold text-stone-400">① 基礎（ワークシート）</p>
              <p className="mt-0.5 text-[18px] font-black tabular-nums text-stone-800">
                {coverage.purpose}
                <span className="text-[13px] font-bold text-stone-400"> / {coverage.total}</span>
              </p>
            ...
            <div className="rounded-2xl bg-accent-soft px-3 py-2.5">
              <p className="text-[13px] font-bold text-accent-dark">② 自分版（これから）</p>
              <p className="mt-0.5 text-[18px] font-black tabular-nums text-stone-800">
                {coverage.ownVersionDone}
                <span className="text-[13px] font-bold text-stone-400"> / {coverage.ownVersionAchievable}</span>
              </p>
              <p className="mt-0.5 text-[13px] leading-relaxed text-stone-500">自分の理解＋自分の質問</p>
            </div>
          </div>
        ...
        <div className="mt-3 flex flex-col gap-2 border-t border-stone-100 pt-3">
          <p className="text-[13px] font-black tracking-widest text-stone-400">次のCheckpoint（未着手）</p>
      ...
      <section className="mt-5 px-5">
        <h2 className="mb-2 text-[14px] font-bold text-stone-400">17フェーズ</h2>
