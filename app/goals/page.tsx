...
function GoalsContent() {
  ...
  return <div className="studio"><StudioHeader eyebrow="MY GOALS" title="なりたい自分" subtitle="遠くの目標と、いまの一歩。自分のペースでつなげていこう。" kind="goals" action={<><button className="studio-primary" onClick={()=>setAdding(true)}>＋ 目標を追加</button><Link href="/goals/journey" className="studio-secondary">道筋を詳しく見る</Link></>} /><StudioConnection />
    ...
    {visible.length===0 ? <div className="studio-card studio-empty"><span className="text-4xl text-violet-300" aria-hidden="true">◎</span><h2 className="font-semibold text-slate-700">まだ、ここに目標はありません</h2><p>目標を追加するか、表示の条件を変えてみてください。</p><button className="studio-secondary" onClick={()=>{setQuery("");setPeriod("すべて");}}>すべての目標を見る</button></div> : <div className="grid gap-5 md:grid-cols-2">{visible.map(g=>{
      ...
      return <button key={g.id} className="studio-card flex flex-col p-6 text-left transition-transform hover:-translate-y-1" onClick={()=>setSelectedId(g.id)}><div className="mb-4 flex w-full items-center justify-between gap-2"><span className="studio-tag">{GOAL_HORIZON_LABEL[g.horizon]}</span><span className="text-xs text-[#877e94]">{g.targetDate?g.targetDate.replaceAll("-","/")+"まで":"日付は未設定"}</span></div><h2 className="text-lg font-semibold leading-8 text-[#3b334b]">{g.title}</h2><p className="mt-3 line-clamp-2 text-sm leading-7 text-[#786f86]">{g.desiredState}</p><div className="mt-auto w-full pt-6"><div className="flex flex-wrap justify-between gap-2 border-t border-[#eeeaf3] pt-4"><span className="text-xs text-[#756b85]">{g.status}</span><span className="text-xs text-[#756b85]">{work.connected && store.executionReady? tasks.length?"関連タスク "+done+" / "+tasks.length+"件完了":"次のタスクは未設定":"関連タスクは接続後に確認"}</span></div>{parent && <p className="mt-3 line-clamp-1 text-xs text-[#8b819b]">↳ {parent.title}</p>}</div></button>;
    })}</div>}
    <p className="mt-6 text-xs leading-6 text-slate-500">タスクの完了数と、目標の達成は別に記録します。</p>
    {selected && <StudioDialog title={selected.title} onClose={()=>setSelectedId(null)}><div className="space-y-6">{!work.connected && <p className="rounded-xl bg-amber-50 p-3 text-xs leading-6 text-amber-800">保存済み初期情報の参考表示です。以下の状況や証拠は最新の実績ではありません。</p>}<div><p className="studio-eyebrow">達成したい状態</p><p className="whitespace-pre-line text-sm leading-8">{selected.desiredState}</p></div><div className="rounded-2xl bg-[#f5f1fb] p-5"><h3 className="mb-2 text-sm font-semibold">達成の基準</h3><p className="whitespace-pre-line text-sm leading-8 text-[#756b85]">{selected.achievementCriteria || "まだ設定していません。"}</p></div>{selected.currentGap && <div><h3 className="mb-2 text-sm font-semibold">いま足りていないこと</h3><p className="text-sm leading-8 text-slate-600">{selected.currentGap}</p></div>}{selected.nextEvidence && <div><h3 className="mb-2 text-sm font-semibold">次に残す証拠</h3><p className="text-sm leading-8 text-slate-600">{selected.nextEvidence}</p></div>}
