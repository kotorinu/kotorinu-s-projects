...
function TasksContent() {
  ...
  return <div className="studio"><StudioHeader eyebrow="MY TASKS" title="やること" subtitle="探して、ひとつ進める。細かな段取りは、タスクを開いてから。" kind="tasks" action={<><button className="studio-primary" onClick={()=>setAdding(true)}>＋ タスクを追加</button><Link className="studio-secondary" href="/tasks/planning">計画・工程を見る</Link></>} /><StudioConnection />
    ...
    <div className="studio-toolbar"><div className="studio-tabs" aria-label="タスクの状態">{tabs.map(x=><button key={x} aria-pressed={tab===x} onClick={()=>setTab(x)}>{x}</button>)}</div><select aria-label="領域で絞り込む" value={area} onChange={e=>setArea(e.target.value)}>{["すべての領域","営業代行","RIALA","GENESIS","Skill Plus","その他"].map(x=><option key={x}>{x}</option>)}</select><select aria-label="期限の月" value={month} onChange={e=>setMonth(e.target.value)}><option value="">すべての月・期限未定</option>{months.map(x=><option key={x} value={x}>{x.replace("-","年")}月</option>)}</select><select aria-label="並び順" value={sort} onChange={e=>setSort(e.target.value)}>{["期限が近い順","名前順"].map(x=><option key={x}>{x}</option>)}</select></div>
    <p className="mb-3 text-xs text-slate-500">{work.connected ? tasks.length+"件" : "保存済み初期情報の参考表示"}{query && "・検索結果"}</p>
    <div className="studio-card overflow-hidden">{tasks.length===0 ? <div className="studio-empty"><span className="text-3xl text-violet-300" aria-hidden="true">⌕</span><h2 className="font-semibold text-slate-700">該当するタスクはありません</h2><p>条件を変えるか、新しいタスクを追加してください。</p><button className="studio-secondary" onClick={()=>{setQuery("");setArea("すべての領域");setMonth("");setTab("すべて");}}>絞り込みを解除</button></div> : tasks.map(t=>{
      const deadline=effectiveDeadline(t,store); const done=isTaskDone(t,store); const blocked=isTaskBlocked(t,store); const overdue=!done && deadline && deadline<store.currentDate; const state=done?(store.completions[t.id]?.metDefinitionOfDone===false?"未達で終了":"完了"):blocked?"待ち":store.taskStartedAt.has(t.id)?"進行中":t.status;
      const theme=AREA_THEME[t.area as keyof typeof AREA_THEME];
      return <article key={t.id} className="group flex items-center gap-3 border-b border-[#eeeaf3] px-4 py-4 last:border-0 sm:gap-4 sm:px-6"><button className="min-w-0 flex-1 text-left" onClick={()=>setSelectedId(t.id)}><div className="flex items-start gap-3"><span aria-hidden="true" className={"mt-1 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border " + (done?"border-[#bbdacc] bg-[#e9f5ee] text-[#56866c]":"border-[#ddd5e9] bg-[#faf8fd] text-[#877896]")}>{done?"✓":state==="進行中"?"•":""}</span><div className="min-w-0"><h2 className={"text-sm font-semibold leading-7 sm:text-[15px] " + (done?"text-slate-500":"text-[#3b334b]")}>{t.title}</h2><div className="mt-2 flex flex-wrap items-center gap-2"><span className="studio-tag" style={theme?{background:theme.soft,color:theme.text}:undefined}>{t.area}</span><span className="text-xs text-[#877e94]">{state}</span><span className={"text-xs " + (overdue?"text-[#b45d64]":"text-[#877e94]")}>{deadline?"期限 "+formatMd(deadline):"期限未定"}</span>{t.estimateMinutes!==null && <span className="text-xs text-[#877e94]">{t.estimateMinutes}分</span>}{t.aiCapability!=="HUMAN" && <span className="studio-tag">{t.aiCapability==="DECISION"?"自分が判断":"AIと進める"}</span>}</div></div></div></button>{work.connected && <button className="studio-icon-button" aria-label={t.title+"を編集"} onClick={()=>setEditTask(t)}>⋯</button>}<span aria-hidden="true" className="hidden text-[#b1a7c0] sm:block">↗</span></article>;
    })}</div>
    <p className="mt-5 text-xs leading-6 text-slate-500">完了・整理した記録も「すべて」から確認できます。</p>
    {selected && <TaskDetailSheet task={selected} onClose={()=>setSelectedId(null)} onNavigateToTask={setSelectedId} />}
