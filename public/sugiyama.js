...
未来を回収して締める|最初の行動と相談先を再確認`.split('\n').map(x => x.split('|'));
  ...
  function parse(text){const matches=[...text.matchAll(/^# (\d+)｜([^\r\n]+).*$/gm)];return matches.map((m,i)=>({id:Number(m[1]),title:m[2],raw:text.slice(m.index,i+1<matches.length?matches[i+1].index:text.length)}));}
  ...
  function render(){
    ...
    let html='<h2>'+p.id+'｜'+esc(p.title)+'</h2><p><strong>目的：</strong>'+esc(s.goal)+'</p><p><strong>完了：</strong>'+esc(s.done)+'</p>'+(s.supplement?'<p class="muted">目的・完了条件には原文をもとにした補助ナビを含みます。セリフは原文です。</p>':'');
    ...
    html+='<details data-script '+(mode==='memory'?'open':'')+'><summary>一語一句の原文を開く</summary>'+renderMarkdown(p.raw,mode==='memory')+'</details>';
    html+='<details><summary>復唱・意味づけ・深掘り・NG・接続</summary><dl class="navigation-notes"><dt>復唱</dt><dd>相手の回答を受け、原文の復唱へ。想定返答を本人の事実にしない。</dd><dt>意味づけ</dt><dd>「という感じですかね？」「この理解で合ってます？」で本人に確認。</dd><dt>深掘り候補</dt><dd>目的がまだ取れていない場合のみ、原文の質問を縦に1〜2問。</dd><dt>返答別分岐</dt><dd>画面下の返答ボタンから該当する原文へ。</dd><dt>NG</dt><dd>解釈の決めつけ、架空の条件・値引き・成果保証はしない。</dd><dt>次への接続</dt><dd>'+esc(phases[index+1]?'次は '+phases[index+1].id+'｜'+phases[index+1].title:'最後の未来回収で終了')+'</dd></dl><p class="muted">この欄は基本ルールに沿った補助表示。具体的なセリフは原文の順序を保持しています。</p></details>';
  ...
  $('#references').onclick=()=>{openDialog('基準資料・原文', '<p>正本：添付「緒方版V5」。参照元の料金資料・動画・実績資料そのものは未添付です。原文の数値を独立検証済みとは扱いません。</p><div class="notice">案件を取る：見せるもの＋実績<br>継続して伸ばす：進め方＋相談環境</div><h3>商品内容の抜け防止</h3><p>2回修正／編集権限／案件保証3件／3案件すべてFB／良い点の言語化／問題＋解決策／具体的リライト例／基礎動画／Slack相談環境／アフター面談／返金保証条件／実績者3名</p><h3>返金保証の原文</h3><p>PF完成後1か月以内に弊社から案件を1件もお渡しできなかった場合は全額返金</p><p>収益保証とは別です。約8倍などの出典注記は原文に保持。未確認の子育て費用は基準値として追加していません。</p><details><summary>基本ルール・前書き</summary>'+renderMarkdown(md.slice(0,md.indexOf('# 1｜')))+'</details>');};
