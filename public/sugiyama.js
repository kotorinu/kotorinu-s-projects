...
未来を回収して締める|最初の行動と相談先を再確認`.split('\n').map(x => x.split('|'));
  ...
  $('#references').onclick=()=>{openDialog('基準資料', '<p>正本：WorkOS「緒方版｜杉山さんロープレ ラポール重視版」（2026年9月24日）。想定回答より本人の発言を優先し、全質問を順番に読み切りません。</p><div class="notice">日常・趣味 → 価値観 → 未来 → 1年後 → 直近目標<br>現状 → 障害 → 認識合わせ → 必要な支援だけ提案</div><h3>価格前の確認</h3><p>内容面の疑問を確認し、本人が役立つ部分を説明できてから総額と正式条件を案内します。収益は保証しません。</p><details><summary>共通の進め方</summary>'+renderMarkdown(md.slice(0,md.indexOf('# 1｜')))+'</details>');};
  ...
  function applyPreview(){phases=parse(displayText(md));index=Math.min(index,Math.max(0,phases.length-1));render();}
  $('#tools').onclick=()=>{
    const syncNotice=canEdit
      ? '<div class="notice"><strong>全端末共通</strong><br>クラウド保存すると、PCとスマホで同じ内容を表示します。</div>'
      : '<div class="notice"><strong>この端末だけ</strong><br>ここでの編集は、このPC内の下書きです。スマホには反映されません。全端末へ反映する変更は正本の更新が必要です。</div>';
    openDialog('編集・設定','<div class="tools"><button id="smaller">文字−</button><button id="larger">文字＋</button><button id="theme">明暗切替</button><button id="speak">このフェーズを読み上げ</button><button id="stopSpeech">停止</button></div><details><summary>Markdownを編集・保存</summary>'+syncNotice+'<p class="muted">全文を1つのMarkdownとして編集します。</p><textarea id="markdown" aria-label="杉山版Markdown"></textarea><p id="editStatus" role="status"></p><div class="tools"><button id="save">'+(canEdit?'クラウド保存':'保存状態を確認')+'</button><button id="preview">プレビューへ</button><button id="export">Markdown書き出し</button><label class="button">Markdown読込<input id="import" type="file" accept=".md,.txt,text/plain,text/markdown" hidden></label><button id="history">履歴・復元</button></div></details>');
  ...
  phases=parse(displayText(md));index=Math.min(Math.max(0,Number(practice.index)||0),phases.length-1);render();
