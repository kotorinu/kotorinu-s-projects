...
# 営業ワークシート反映と、実績の手入力（2026-09-08 第3ラウンド）

## ①基礎が埋まった — フェーズ名も正本へ修正

本人から「オンライン営業特化：トーク作成ワークシート」PDFが共有された。
17フェーズ分の 目的 / OK状態（次へ進む条件） / 確認事項 / 質問例（参考） /
NG例 をそのまま転記し、`salesPhases` の①基礎が 0/17 → **17/17** になった。

**フェーズ名と順番も修正した。** 従来のfixtureはワークシートと一致して
おらず、3番目の「会話主導権形成（アジェンダ設定）」が抜け、代わりに実在
しない「決済」が17番目に入っていた。3以降が1つずつズレていたことになる。
ワークシートが正本なので上書きし、実践者FBの紐付けも同じだけシフトした。

| | 旧fixture | ワークシート（正本） |
|---|---|---|
| 03 | 現状把握 | **会話主導権形成（アジェンダ設定）** |
| 04 | 理想の明確化 | 現状把握 |
| … | （1つずつズレ） | … |
| 17 | 決済 | クロージング（オンライン契約） |

## 進捗の見方を「基礎」と「自分版」へ分けた

①基礎はワークシートから来たもので、埋まっていても本人の進捗ではない。
進捗として見るのは **②自分版**（自分の理解＋自分の質問＝ワークシートの
【ワーク】欄）。

- ① 基礎（ワークシート）: **17 / 17**
- ② 自分版（これから）: **0 / 11**

分母が11なのは、商品情報が必要な6フェーズ（10 解決可能性提示 / 11 解決策
提示 / 12 解決策理解 / 15 テスクロ / 16 オファー / 17 クロージング）を
`PRODUCT_INFO_REQUIRED` として分母から外したため。商品レクチャー待ちで
埋まらないことを、進捗の遅れとして数えない。

## Taskを「読む」から「自分の言葉にする」へ

基礎が埋まったので、9/8-9/9のTaskの中身が変わった。

- 9/8 夜「17フェーズの目的とOK状態を、**自分の言葉で言い直す**」
- 9/9 夜「ワークシートの**【ワーク】欄を埋め、自分の質問を作る**」
- 9/9 昼「自分版の詰まりを口頭で洗い出し、夜に書く箇所を3つに絞る」

動画Taskは実行計画から外した。本人の指示は「動画は正直参照程度。自分の
営業の型を決める方が先」。

## 営業動画は参照素材として登録（優先度：低）

タイトルが判明した4本（実践編①ラポール26分 / ②ヒアリング24分 /
③クロージング26分 / アフターフォロー・データ分析35分）を該当Phaseへ
紐づけて登録。残り8本は未共有なので作らない。`priority: "参照のみ"` を
持たせ、視聴を先に埋める計画をAIが組めないようにした。

## 実績時間の手入力（components/ManualActualEntry.tsx）

開始/完了を押し忘れると、見積り改善の材料になる実績が貯まらない。Task
Detailの「予定・実績」から、後から分数を入力できるようにした（プリセット
15〜180分＋自由入力）。入力値はタイマー計測と区別して「手入力」と表示し、
completionレコードのactual/varianceも同時に更新する。推測値は入れない。

## Area Homeをスマホ向けに作り直し

初版は 目的 / GOAL / Outcome / 現在地 の長文4連発が先頭にあり、スマホでは
何を見る画面か分からなかった。順番を **追っている数字 → いま必達 →
次にやること（時刻付き）** に変え、説明文・Source・Blocker・達成条件は
折りたたみへ移した（消してはいない。件数はヘッダーに出す）。

各Areaの「追っている数字」は1つだけ：
